...
template<class X, class Y> void add(X &x, const Y &y) {x+=y; }
int n , k ;
void init(void) {
	cin >> n >> k ;
}
unsigned long long dp[2][305][305] ;  
void process(void) {
    dp[0][0][0] = 1 ; 
    ll ans = 0 ;
    FOR(i,1,n) {
    	...
    	FOR(t1,0,k) FOR(t2,1,k) add(dp[cur][0][t2],dp[prev][t1][t2-1]) ; 
    	FOR(t1,1,k) FOR(t2,0,k) add(dp[cur][t1][0],dp[prev][t1-1][t2]) ; 
    	if(i==n) {
    		FOR(t1,0,k) FOR(t2,0,k) add(ans,dp[cur][t1][t2]) ; 
    	}
